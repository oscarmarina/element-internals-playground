...
export class BlkInput extends BlkMixinFormAssociated(LitElement) {
   ...
   */
  @property({type: String, reflect: true, useDefault: true, attribute: 'enterkeyhint'})
   ...
   */
  @property({type: String, reflect: true, useDefault: true, attribute: 'inputmode'})
  override inputMode:
    | 'none'
    | 'text'
    | 'tel'
  ...
  override async connectedCallback() {
    ...
    this.__labels = this._computedLabelText;
  ...
  get _computedLabelText() {
    return Array.from(this.labels as NodeListOf<HTMLElement>).reduce(
  ...
  get _fieldTpl() {
    return html`
      <div class="field-flex">
        <div class="field">${this._labelTpl} ${this._inputTpl}</div>
      </div>
    `;
  }
  ...
  get _labelTpl() {
    return html`
      ${this.label ? html` <label for="${this.__internalIdref}">${this.label}</label> ` : nothing}
    `;
  }
  ...
  private _onKeydown(ev: KeyboardEvent) {
    const {key} = ev;
    if (['Tab', 'Shift', 'Meta', 'Alt', 'Control'].includes(key)) {
      return;
    }
